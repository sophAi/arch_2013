...
以下程式均為敝人所撰寫，以GNU License (GPL)發佈。(**點選每個子標題可以連結到軟體GitHub專頁**)
...
以GPGPU進行向量函數自相關(vector autocorrealtion)的平行化加速計算: 對於一系列向量，(a) 將序列資料依序讀進odd或even buffer; (c) 將自相關函式的kernel編譯並上傳至GPU的work item，(b) 對buffer裡的序列資料進行計算。(**點圖可放大**)


CL-VAF [`1`_, `2`_, `4`_, `5`_] 是一個以C/C++/OpenCL撰寫的計算程式，透過GPGPU的平行化對隨時間變化的向量進行自相關函數(autocorrelation function)的計算，該程式由MPICH版本的\ `TCOM`_\ 修改而來。與Nvidia的CUDA不同在於，OpenCL具有更普遍的相容性，其支援Nvidia、AMD、Intel、Apple甚至是ARM系統，此程式的效能評估是在消費等級的顯示卡上完成的，在更先進的機器上，例如Tesla應該更能發揮其平行化的優勢。
...
D-Tools是一個以C/C++撰寫的程式，程式碼約為1000多行，是一個用來輔助Diffusion Theory理論計算的工具程式，這是我的博士論文中，與義大利 Arnaldo Rapallo 博士一起進行的題目，在完成PTMD後，我將程式設計的重心完全轉移至C/C++與Python，這是當時的完成品之一。
...
TCOM [`1`_, `2`_, `4`_, `5`_] 是MPICH版本的向量自相關函式計算程式，完成後不久，我以該程式為雛型設計了GPGPU的版本 (CL-VAF)。
...
* 教育部Ezgo專案的PhET線上教育推廣，物理實驗軟體的中文翻譯者，翻譯作品如下:
...
* 應用統計(Python)，時間序列分析(time series analysis)，數值模擬(Matlab/Octave)課程教學。
